...
$users =Contactus::where('status','=','new')
                   ->where('priority','=','low')
                  ...
                  ->get();
                 dd($users);
                //  $tdate=Carbon::parse($users[0]->created_at);
                //  dd($tdate);
                 $diff = Carbon::parse($users[0]->created_at)->diffForHumans(Carbon::now());
                // dd($diff);
                 
                 if($diff == 1){
                    //$priority = Contactus::where('priority', 'low')->where('created_at', '<=', now())->update(['priority' => 'medium']);
                      echo("medium");
                     //dd($priority);
                 }elseif($diff == 2){
                   // $priority = Contactus::where('priority', 'low')->where('created_at', '<=', now())->update(['priority' => 'high']);
                    echo("high");
                 }
                 else{
                 
                    // $priority = Contactus::where('priority', 'low')->where('created_at', '<=', now())->update(['priority' => 'low']);
                    // dd($priority);
                    echo("low");
                 }
